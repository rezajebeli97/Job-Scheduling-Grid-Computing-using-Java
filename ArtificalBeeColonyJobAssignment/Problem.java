...
public class Problem {
	...
	public State randomState() {
		...
		return new State(timeToLive , solution);
	}
	...
	public ArrayList<State> initialStates(int numberOfInitialStates) {
		...
		for (int k = 0; k < numberOfInitialStates; k++) {
			...
			initialStates.add(new State(timeToLive , solution));
		}
		return initialStates;
	}
	
	public ArrayList<State> nextStates(ArrayList<State> states){
		ArrayList<State> nxtStates = new ArrayList<State>();
		for (State state : states) {
			if (state.getTimeToLive() > 0) {
				State neighbourState = state.mutant(timeToLive , this , states.get((int) (Math.random()*states.size())));
				if (f(neighbourState) >= f(state))
					nxtStates.add(neighbourState);
				else {
					state.setTimeToLive(state.getTimeToLive() - 1);
					nxtStates.add(state);
				}
			}
		}
