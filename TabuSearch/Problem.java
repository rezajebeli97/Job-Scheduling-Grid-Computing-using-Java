//با فرض آنکه استیت بهتر دارای اف بزرگتر است.
...
public class Problem {
	...
	public ArrayList<Action> actions(State state) {
		...
		for (int i = 0; i < jobs.length; i++) {
			for (int j = 0; j < clusters.length; j++) {
				if (state.getSolution()[i] == j)
					continue;
				int remainedCore = clusters[j].core;
				int remainedRam = clusters[j].ram;
				for (int k = 0; k < jobs.length; k++) {
					if (state.getSolution()[k] == j) {
						remainedCore -= jobs[k].core;
						remainedRam -= jobs[k].ram;
					}
				}
				if (remainedCore >= jobs[i].core && remainedRam >= jobs[i].ram)
					actions.add(new Action(i, j, 1));
	...
	public State result(State state, Action action) {
		...
		int[] solution = new int[jobs.length];
		for (int i = 0; i < solution.length; i++) {
			solution[i] = state.getSolution()[i];
		}
		solution[action.getJobIndex()] = action.getClusterIndex();
	...
	public boolean contains(ArrayList<State> myPath, State neighbour) {
		for (State state : myPath) {
			if (this.equal(state, neighbour)) {
				return true;
			}
		}
		return false;
